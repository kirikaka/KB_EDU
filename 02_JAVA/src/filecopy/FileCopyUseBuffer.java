...
public class FileCopyUseBuffer {
	...
	public static void main(String[] args) {
		...
		File source = new File("./05_IO/covid.txt");
		
		long start = System.currentTimeMillis();
		int count = 0;
		while(count++ < 100) {
			FileCopy.fileCopyStreamVer(source, new File("/05_IO/c_covid"+count+".txt"));
		}
		...
		count = 0;
		while(count++ < 100) {
			FileCopyUseBuffer.fileCopyBufferdStreamVer(source, new File("/05_IO/c2_covid"+count+".txt"));
		}
